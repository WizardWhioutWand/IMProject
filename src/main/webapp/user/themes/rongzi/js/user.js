...
function checkSms() {
	...
	if (phone == "" || !phone) {
		...
		if (!reg.test(phone)) {
			alert("请填写11位以13、15、18或者14、16、17、19开头的数字正确手机号码!");
			return false;
		}
		alert("短信验证码正在发送，请注意接收!");
	}
	$.post("index.php?user&q=reg&type=sendMessage", {
		phone : phone
	}, function(result) {
		if (result == true) {
			alert("手机号码已存在！");
...
function checkPassword(password) {
	...
	if (password.length < 6 || password.length > 20) {
		...
		submit_disabled = true;
	}else if(pattern.test(s) == false){
		submit_disabled = true;	
		document.getElementById('password_notice').innerHTML = password_msg_shorter_shuz;
...
function checkConformPassword(conform_password) {
	...
	if (conform_password.length < 6) {
		...
		document.getElementById('conform_password_notice').innerHTML = password_msg_shorter_shuz;
		submit_disabled = true;
	}
...
var msg_can_reg = "<img src='./themes/rongzi/images/42.gif' />";
var username_msg = ' 请输入3-15位字符';
...
var phone_msg_empty = " 手机号码为空";
var phone_msg_invalid = " 手机号码不是合法的号码";
var phone_msg_blank = " 手机号码不能为空";
var phone_msg_exist = " 手机号码已存在,请重新输入";
var phone_msg_format = " 手机号码不合法";

var sms_msg_blank = " 手机验证码不能为空";
var sms_msg_format = " 手机验证码不合法";
var sms_msg_notequal = " 手机验证码错误，请重新获取";


var password_msg_shorter = " 登录密码不能少于 6 个字符";
var password_msg_shorter_shuz = " 密码必须是字母和数字的组合";
