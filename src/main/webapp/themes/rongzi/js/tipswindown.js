...
function tipsWindown(title, content, width, height, drag, time, showbg, cssName, dom) {
	if(dom != undefined) {
		var dom1 = $(dom).parent().find('font').text();
		if(dom1 != null && dom1 != "待上传" && dom1 != "未通过，请重新上传") {
			if(dom1 != null && dom1 != "待上传" && dom1 != "未通过，请重新上传" && dom1 == "待审核") {
				alert("管理员正在审核，如需要管理员尽快操作请联系管理员!");
				return false;
			} else {
				alert("您的材料已审核通过，不需要再重复提交!");
				return false;
			}
		}
	}
