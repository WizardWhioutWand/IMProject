...
function getCheckBoxValues(c) {
	var result = '';
	$("input[name='" + c + "']").each(function() {
		result += $(this).attr("checked") ? $(this).val() + "," : "";
	});
...
function ppup(msg, c, fn, img, t, y, n) {
	...
	$.cbox('<div style="top:20px;left:10px;position:relative;float:left;"><image align="absmiddle" src="' + img + '" /></div><div style="left:30px;top:25px;position:relative;float:left;height:60px;width:200px;font-size:14px;">' + msg + '</div><div style="right:18px;bottom:18px;position:absolute;padding:0;float:right">' + bt + '</div>', {
		tt: t
	});
	if(fn) {
		$('.cconfirmY').click(function() {
			fn(true)
		});
		$('.cconfirmN').click(function() {
			fn(false)
		});
		$('.calertC').click(function() {
			fn(true)
		});
	}
}
